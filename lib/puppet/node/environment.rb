...
class Puppet::Node::Environment
  ...
  # Need the list of modules not uniqued and even the invalid ones
  def modules_by_path
    modules_by_path = {}
    modulepath.each do |path| 
      Dir.chdir(path) do
        module_names = Dir.glob('*').select { |d| FileTest.directory? d }
        modules_by_path[path] = module_names.map {|name| Puppet::Module.new(name, self, File.join(path, name)) }
      end
    end
    modules_by_path
  end
